...
> http://8.148.8.210:9981/ 可以前往该网址下载构建完成的安装包
...
1. 将 `TubaToolbox.exe` 放到图吧工具箱的 `tools` 文件夹同级目录
2. 创建 `config.json` 配置文件（格式见下方）
3. 双击运行即可
...
```json
{
  ...
  "Tools": [
    {
      "Name": "CPU-Z",
      "Description": "CPU信息检测工具",
      "Category": "CPU工具",
      "RelativePath": "tools\\CPU工具\\CPU-Z.exe",
      "Icon": "⚡",
      "IsImage": false,
      "IsInfoOnly": false
    }
...
```bash
# 克隆项目
git clone https://github.com/zds745926/TubaToolbox.git
cd TubaToolbox
